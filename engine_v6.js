// ==========================================
// ENGINE.JS - GLOBAL SPECTACLE & OPTIMIZED EDITION (V40.0)
// [ĐỈNH CAO: 100% ENGLISH, SIÊU MƯỢT 60FPS, CHỐNG TRÀN RAM, DOM CACHING]
// ==========================================
...
// Biến lưu trữ tối ưu hóa (DOM Caching)
window.lastH1 = ""; window.lastH2 = ""; window.lastS1 = "";
window.lastBassTime = 0;

// 🌟 HỆ THỐNG ÂM THANH EPIC TỐI ƯU
window.playSound = function(freq, type, duration, vol, isImpact = false) { 
    ...
    try {
        ...
        // Tối ưu hóa: Chỉ dội Bass 1 lần mỗi 300ms để tránh kẹt CPU thanh âm
        if (freq < 150 && isImpact && (Date.now() - window.lastBassTime > 300)) {
            ...
            bassOsc.frequency.exponentialRampToValueAtTime(20, t + duration * 1.5);
            bassGain.gain.setValueAtTime(safeVol * 1.2, t);
...
// Tối ưu số lượng hạt
window.spawnParticles = function(x, y, color, isCrit = false) { let count = isCrit ? 15 : 8; for(let i=0; i<count; i++) { let angle = Math.random() * Math.PI * 2; let speed = Math.random() * (isCrit?15:6) + 2; window.particles.push({ x: x, y: y, vx: Math.cos(angle)*speed, vy: Math.sin(angle)*speed, life: 25, maxLife: 25, color: color, size: Math.random() * 4 + 2 }); } }
window.spawnScreenBlood = function() { for(let i=0; i<2; i++) { window.screenBlood.push({ x: Math.random()*800, y: Math.random()*600, size: 20 + Math.random()*40, alpha: 0.8, vy: 0.5 + Math.random() }); } }
window.spawnDamageNumber = function(x, y, text, color, isCrit) { window.floatingTexts.push({ x: x, y: y, text: text, color: color, alpha: 1.5, vx: (Math.random() - 0.5) * 8, vy: -(Math.random() * 6 + 5), font: `900 ${isCrit ? 50 : 35}px Impact`, life: 60, isPhysics: true }); };

window.spawnSpeechBubble = function(x, y, text, isLeft = false) { window.speechBubbles.push({ x: x, y: y, text: text, isLeft: isLeft, life: 60, maxLife: 60 }); };
window.spawnDebris = function(x, y, count) { let safeCount = Math.min(count, 15); for(let i=0; i<safeCount; i++) { window.debris.push({ x: x + (Math.random()-0.5)*80, y: y + 50, vx: (Math.random()-0.5)*20, vy: -(Math.random()*12 + 8), rot: Math.random()*Math.PI, vRot: (Math.random()-0.5)*0.5, size: Math.random()*12 + 5, life: 80 }); } };
...
window.triggerFatality = function(target, isSmash = false) {
    ...
    window.matchResolved = true; window.gameOver = true; window.koGlitchTimer = 80; window.hitZoomTimer = 100; window.fatalBlowFlash = 10; 
...
window.moveFPS = function(dir) {
    if (window.superArtTimer > 0 || window.playerFPS.stamina < 5 || window.gameOver || window.playerFPS.isDodging || window.playerFPS.guardBreakTimer > 0 || window.clashStruggleTimer > 0) return;
...
window.punch = function(hand) {
    ...
    if (e && e.hp > 0 && window.enemyZ <= 90) { 
        ...
        if (e.hp <= 0) {
            ...
            if (!isRagePunch) { let hitStopDuration = (isCrit || isPerfectCounter) ? 60 : 0; if (hitStopDuration > 0) { window.isLoopRunning = false; setTimeout(() => { if(!window.gameOver) { window.isLoopRunning = true; requestAnimationFrame(window.gameLoopFPS); } }, hitStopDuration); } }
...
window.blockFPS = function() { 
    if (window.superArtTimer > 0 || window.playerFPS.stamina <= 0 || window.gameOver || window.playerFPS.guardBreakTimer > 0 || window.clashStruggleTimer > 0) return; 
...
window.dodgeFPS = function() { 
    if (window.superArtTimer > 0 || window.playerFPS.stamina < 20 || window.playerFPS.isDodging || window.gameOver || window.playerFPS.guardBreakTimer > 0 || window.clashStruggleTimer > 0) return; 
    ...
    if (isPerfect) { 
        ...
        window.camZoom = 1.25; window.cameraTilt = window.playerFPS.dodgeDir * 0.3;
...
window.update = function() {
    ...
    // TỐI ƯU RÁC (CULLING) ĐỂ GIỮ FPS MƯỢT MÀ
    if (window.particles.length > 80) window.particles.splice(0, window.particles.length - 80);
    if (window.floorSplatters.length > 40) window.floorSplatters.splice(0, window.floorSplatters.length - 40);
    if (window.bloodPools.length > 10) window.bloodPools.splice(0, window.bloodPools.length - 10);
    if (window.debris.length > 30) window.debris.splice(0, window.debris.length - 30);
    ...
    if (window.lastH1 !== h1W) { if(h1) h1.style.width = h1W; window.lastH1 = h1W; }
    if (e) {
        let h2W = (e.hp / e.maxHp * 100).toFixed(1) + "%";
        if (window.lastH2 !== h2W) { if(h2) h2.style.width = h2W; window.lastH2 = h2W; }
    }
    let s1W = window.playerFPS.stamina.toFixed(1) + "%";
    if (window.lastS1 !== s1W) { if(s1) s1.style.width = s1W; window.lastS1 = s1W; }
...
// ==========================================
// 4. VÒNG LẶP VẼ (DRAW ĐIỆN ẢNH) - ĐÃ BỎ BỚT LỌC MÀU GÂY LAG
// ==========================================
window.draw = function() {
    ...
    if (window.superArtTimer > 0 && window.superArtData) {
        ...
        for(let i=0; i<80; i++) { let ang = Math.random() * Math.PI * 2; let r1 = 0; let r2 = 1500; window.ctx.moveTo(Math.cos(ang)*r1, Math.sin(ang)*r1); window.ctx.lineTo(Math.cos(ang)*r2, Math.sin(ang)*r2); }
        window.ctx.stroke();
        ...
        window.ctx.rotate(0.15); 
        window.ctx.fillStyle = "#fff"; window.ctx.shadowBlur = 10; window.ctx.shadowColor = "#000";
    ...
    // TỐI ƯU CỰC MẠNH: Thay vì dùng ctx.filter (giết FPS), dùng Overlay phủ màu
    if (window.blackoutTimer > 0 || (window.gameOver && window.matchResolved) || window.fatalBlowFlash > 0) { 
        // Vẫn giữ một chút filter nếu thực sự cần thiết, nhưng dùng ít đi.
        if (window.blackoutTimer > 0) window.ctx.filter = "invert(100%) grayscale(100%)"; 
        else if (window.fatalBlowFlash > 0) window.ctx.filter = "invert(100%)";
    }
    ...
    window.bloodPools.forEach(bp => { if (bp.size < bp.maxSize) bp.size += 0.2; window.ctx.save(); let perspectiveScale = 300 / (200 + window.enemyZ); window.ctx.translate(bp.x, bp.y); window.ctx.scale(perspectiveScale, perspectiveScale * 0.3); window.ctx.fillStyle = "rgba(139, 0, 0, 0.75)"; window.ctx.beginPath(); window.ctx.arc(0, 0, bp.size, 0, Math.PI*2); window.ctx.fill(); window.ctx.restore(); });
    window.debris.forEach(d => { window.ctx.save(); window.ctx.translate(d.x, d.y); window.ctx.rotate(d.rot); window.ctx.fillStyle = "#34495e"; window.ctx.strokeStyle = "#111"; window.ctx.lineWidth = 1; window.ctx.beginPath(); window.ctx.rect(-d.size/2, -d.size/2, d.size, d.size); window.ctx.fill(); window.ctx.stroke(); window.ctx.restore(); });
    ...
    // TỐI ƯU SPEEDLINES: Giảm 80 dòng xuống 30 dòng (Đỡ lag gấp 3 lần)
    if (window.speedLinesAlpha > 0) { window.ctx.save(); window.ctx.translate(window.canvas.width/2, window.canvas.height/2); window.ctx.strokeStyle = `rgba(255, 255, 255, ${window.speedLinesAlpha})`; window.ctx.lineWidth = 1.5; window.ctx.beginPath(); for(let i=0; i<30; i++) { let ang = Math.random() * Math.PI * 2; let r1 = 250 + Math.random()*200; let r2 = 1200; window.ctx.moveTo(Math.cos(ang)*r1, Math.sin(ang)*r1); window.ctx.lineTo(Math.cos(ang)*r2, Math.sin(ang)*r2); } window.ctx.stroke(); window.ctx.restore(); }
    ...
    // TẮT SHADOW BLUR CỦA CHỮ ĐỂ CHỐNG LAG FPS
    window.floatingTexts.forEach(t => { window.ctx.font = t.font || "900 22px Arial"; window.ctx.fillStyle = t.color; window.ctx.globalAlpha = Math.max(0, t.alpha); window.ctx.textAlign = "center"; window.ctx.fillText(t.text, t.x, t.y); window.ctx.strokeText(t.text, t.x, t.y); }); window.ctx.globalAlpha = 1.0;

    if (window.destructiveFinishTimer > 0) { window.destructiveFinishTimer--; window.ctx.save(); window.ctx.translate(window.canvas.width/2, window.canvas.height/2 - 50); let scale = 1 + (200 - window.destructiveFinishTimer) * 0.01; window.ctx.scale(scale, scale); window.ctx.font = "900 180px 'Yu Mincho', 'MS Mincho', serif"; window.ctx.textAlign = "center"; window.ctx.textBaseline = "middle"; window.ctx.fillStyle = `rgba(255, 0, 60, ${window.destructiveFinishTimer / 200})`; window.ctx.fillText("滅", 0, 0); window.ctx.restore(); }
    ...
    // Lớp phủ đen khi Game Over (Nhẹ hơn dùng filter rất nhiều)
    if (window.gameOver && window.matchResolved) { window.ctx.fillStyle = "rgba(0, 0, 0, 0.4)"; window.ctx.fillRect(0, 0, window.canvas.width, window.canvas.height); window.ctx.fillStyle = "#000"; window.ctx.fillRect(0, 0, window.canvas.width, 80); window.ctx.fillRect(0, window.canvas.height - 80, window.canvas.width, 80); }
